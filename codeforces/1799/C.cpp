...
signed main() {
    ...
    while(t--){
        ...
        for(int i =0; i<m;i++){
            if(arr[i].second %2 ==0){
                string  str(arr[i].second/2,arr[i].first);
                y+= str;
            }
            else{
                string  str(arr[i].second/2,arr[i].first);
                ...
                if(i<m-2){
                    ...
                    i++;
                    while(i<m){
                        string str2(arr[i].second,arr[i].first);
                        center+=str2;
                        i++;
                    }
                    center+=arr[k].first;
                    ...
                    if(i==m-2){
                        string left((arr[m-1].second+1)/2, arr[m-1].first);
                        string right((arr[m-1].second)/2, arr[m-1].first);
                        center = left;
                        center+=(arr[i].first+right);
                        i++;
                    }
                    else{
                        center = arr[i].first;
