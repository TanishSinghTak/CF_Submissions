...
signed main() {
    ...
    while(t--){
        ...
        if(n==1){
            ...
            int ans = (abs(arr[0])+abs(arr[1])+abs(arr[2])+abs(arr[3]));
            ans = min (ans,(abs(arr[0]-2)+abs(arr[1]-2)+abs(arr[2]-2)+abs(arr[3]-2)) );

            ans = min (ans,(abs(arr[0]+1)+abs(arr[1]+1)+abs(arr[2]+1)+abs(arr[3]-2)) );
            ans = min (ans,(abs(arr[0]+1)+abs(arr[1]+1)+abs(arr[2]-2)+abs(arr[3]+1)) );
            ans = min (ans,(abs(arr[0]+1)+abs(arr[1]-2)+abs(arr[2]+1)+abs(arr[3]+1)) );
            ans = min (ans,(abs(arr[0]-2)+abs(arr[1]+1)+abs(arr[2]+1)+abs(arr[3]+1)) );
            ...
            if(n%2 ==1){
                ...
                ans = LLONG_MAX;
                for(int i =0; i<2*n; i++){
                    ans = min(ans, k + abs(arr[i]-n) - abs(arr[i]+1));
                }
                ans = min(ans,ans2);
                cout<<ans<<endl;
