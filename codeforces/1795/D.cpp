...
const int N = 1e6; //change to 32000 if TLE happens for prime factors
...
signed main() {
    ...
    // cout.precision(30);
        ...
        int k = n/6;
        for(int i =0; i<2*k; i++){
            int x = arr[3*i]+arr[3*i +1];		
            int y = arr[3*i +2]+arr[3*i +1];			
            int z = arr[3*i]+arr[3*i +2];	
			int maxn = max(x,y);
			maxn = max(maxn,z);		
            if(x==y && y==z){
                prod = (prod*3)%mod;
            }
            else if(((x==y || x==z)&&(x == maxn)) || (y==z && y == maxn)){
                prod = (prod*2)%mod;
            }
        }
        int ans = (nCr(2*k,k,mod)*prod)%mod;
        cout<<ans<<endl;
